...
<br>
💿[웹  시연영상](https://youtu.be/oRULA-Fp-iE)  
...
|한승우|NCP 서버 전개 및 배포<br>NCP 로드밸런서 생성 및 배포<br>아임포트(I'mport)을 활용한 결제 기능 구현<br>주문 관련 기능 구현<br>카카오 우편번호 API를 이용한 검색 기능 구현<br>사용자 배송지 선택,생성,삭제 및 기본배송지 설정 구현<br>장바구니, 배송지와 주문 연동 구현<br>Spring Security를 이용한 Bcrypt 암호화 작업<br>CLOVA chatbot의 JSON 데이터 가공<br>나의 주문리스트 페이지 구현<br>주문 상세 페이지 구현<br>관리자 페이지 상품,공지/이벤트 관리(CRUD) 및 이미지 파일 관련 기능 구현<br>관리자 페이지 실시간 통계 기능 구현<br>(매출차트,나이대별/성별 판매비율, 카테고리별 판매비율,주문당 평균)|
