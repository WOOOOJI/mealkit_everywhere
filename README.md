...
|이름|역할|
|--|--| 
|김기태|상품 리스트 페이지 구현<br>게시판 페이지네이션 구현<br>상품 상세 페이지 구현<br>문의 - 후기 게시판(CRUD) 구현<br>챗봇 API 및 WebSocket을 이용한 문의 처리 구현<br>관리자 회원관리 게시판 구현<br>관리자 실시간 및 연월일 통계 기능 구현(전일 대비 판매 차트, TOP-WORST10 상품 리스트)|  
...
|한승우|아임포트(I'mport)을 활용한 결제 기능 구현<br>주문 관련 기능 구현<br>카카오 우편번호 API를 이용한 검색 기능 구현<br>사용자 배송지 선택,생성,삭제 및 기본배송지 설정 구현<br>장바구니, 배송지와 주문 연동 구현<br>Spring Security를 이용한 Bcrypt 암호화 작업<br>나의 주문리스트 페이지 구현<br>주문 상세 페이지 구현<br>관리자 페이지 상품,공지/이벤트 관리(CRUD) 및 이미지 파일 관련 기능 구현<br>관리자 페이지 실시간 통계 기능 구현<br>(매출차트,나이대별/성별 판매비율, 카테고리별 판매비율,주문당 평균)|
...
|:--:|--|--|
|김기태|⦁ 문제점1: 상품 상세페이지 내에 문의-후기 게시판 각각 구현 시, 동기방식으로는 구현 불가능<br><br>⦁ 문제점2: ThymeLeaf 반복문으로 나열한 페이지 번호 중, 현재 페이지 번호에만 CSS를 적용해야하는 상황 발생<br><br>⦁ 문제점3: 팝업창 실행시 JQuery에서 Uncaught ReferenceError : functionName is not defined 에러 발생|⦁ 솔루션1: AJAX를 이용하여 데이터를 JSON객체에 담고, 각 게시판을 비동기 통신하여 해결<br><br>⦁ 솔루션2: 각 페이지 번호의 a태그에 페이지 번호를 id로 주어, 특정 페이지에 CSS를 부여함<br><br>⦁ 솔루션3: DOM 구조가 동적으로 변경될 때, JQuery는 처음 DOM 구조가 완료되고 난 뒤에 동작함 -> onclick시 실행되는 팝업창을 여는 함수를 $(document).ready 밖으로 빼내어 해결| 
...
|이상아|⦁ 문제점1: 회원가입폼에 데이터를 입력하면 placeholder로 표시한 텍스트들이 작아져야 하는데 실패, 폼 수정하는 과정에서 위치가 섞이면서 스타일적용이 안됨<br><br>⦁ 문제점2: 문의 및 답변을 구현하려고 할 때 org.apache.ibatis.binding.BindingException: Invalid bound statement (not found): com.shop.mapper.CommentsMapper.qnaReply 오류 발생 |⦁ 솔루션1: <label>과 <input>태그 위치를 서로 바꾸고, id값과 laber for값을 일치시킴<br><br>⦁솔루션2: mapper.xml에서 <mapper namespace="com.shop.mapper.CommentMapper">의 Comment를 Comments로 수정하여 해결 | 
