...
|:--:|--|--|
|김기태|⦁ 문제점1: 상품 상세페이지 내에 문의-후기 게시판 각각 구현 시, 동기방식으로는 구현 불가능<br><br>⦁ 문제점2: ThymeLeaf 반복문으로 나열한 페이지 번호 중, 현재 페이지 번호에만 CSS를 적용해야하는 상황 발생|⦁ 솔루션1: AJAX를 이용하여 데이터를 JSON객체에 담고, 각 게시판을 비동기 통신하여 해결<br><br>⦁솔루션2: 각 페이지 번호의 a태그에 페이지 번호를 id로 주어, 특정 페이지에 CSS를 부여함| 
...
|이상아|⦁ 문제점1: 회원가입폼에 데이터를 입력하면 placeholder로 표시한 텍스트들이 작아져야 하는데 실패, 폼 수정하는 과정에서 위치가 섞이면서 스타일적용이 안됨<br>⦁ 문제점2: 문의 및 답변을 구현하려고 할 때 org.apache.ibatis.binding.BindingException: Invalid bound statement (not found): com.shop.mapper.CommentsMapper.qnaReply 오류 발생 |⦁ 솔루션1: <label>과 <input>태그 위치를 서로 바꾸고, id값과 laber for값을 일치시킴<br>⦁솔루션2: mapper.xml에서 <mapper namespace="com.shop.mapper.CommentMapper">의 Comment를 Comments로 수정하여 해결 | 
|한승우|⦁문제점1: AJAX를 이용하여 JavaScript에서 변수에 값을 담으려고 하였지만, AJAX 통신 내에서만 데이터가 존재하고 AJAX 밖으로 나가면서 사라지는 현상 발생<br>⦁ 문제점2: RealTimeAnalyzeService(실시간 통계)에서 Java의 Date를 통해 현재 시스템의 시간을 구해서 변수로 선언하였지만 배포시 실시간으로 시간이 변하지는 않았음<br>⦁문제점3: 날짜별/시간별 매출 데이터를 조회하는 쿼리문을 작성했을 때 데이터가 없는 날짜/시간은 0으로 나오는 것이 아니라 레코드 자체가 존재하지 않아 데이터를 바로 사용할 수 없었음|⦁솔루션1: 'async:false'를 이용하여 변수에 담을 수 있게 통신 방식을 변경하였고, AJAX 통신 전에 전역변수로 선언해준 후 값을 전달하여 사용<br>⦁솔루션2: Service는 Controller에서 호출될 때마다 생성되고 사라지는 것이 아니라 @Bean으로 한 번 생성되는 것이기 때문에 Service 안에 getTime()이라는 메서드를 만들어 통계 메서드 실행 시 getTime() 메서드를 호출하여 시간 변경<br>⦁솔루션3: 날짜/시간에 대한 더미 테이블을 생성한 후 OUTER JOIN을 이용해 데이터가 없어도 레코드가 존재하게 설정| 
